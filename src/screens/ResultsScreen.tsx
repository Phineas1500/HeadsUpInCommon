    ...
    >
      ...
      {round.guessedNames.length > 0 && (
        <div className="flex-1 overflow-y-auto w-full max-w-md px-6 pb-4">
          <h3 className="text-sm font-semibold text-overlay uppercase tracking-wider mb-3">
            Guessed
          </h3>
          <div className="space-y-2">
            {round.guessedNames.map((name, i) => (
              <motion.div
                ...
                className="bg-surface-light rounded-xl px-4 py-2.5 flex items-center justify-between"
              >
                <span className="text-text font-medium">{name}</span>
                <span className="text-overlay text-sm">#{i + 1}</span>
              </motion.div>
            ))}
          </div>
        </div>
